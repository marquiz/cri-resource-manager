...
JEKYLL_VERSION := 3.8
DOCKER_SITE_CMD := $(DOCKER) run --rm -it --volume="$$PWD/docs:/srv/jekyll" --volume="$$PWD/docs/vendor/bundle:/usr/local/bundle" --network=host jekyll/jekyll:$(JEKYLL_VERSION)
...
site-build:
	$(Q)$(DOCKER_SITE_CMD) jekyll build

site-serve:
	$(Q)$(DOCKER_SITE_CMD) jekyll serve -H 127.0.0.1 -b /
